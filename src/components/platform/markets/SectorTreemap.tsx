...
export default function SectorTreemap({
  ...
  return (
    ...
    >
              ...
              {sectorRect.children?.map((stockRect) => {
                ...
                if (analysisMode === 'strategy') {
                  ...
                  if (stratTradesCount > 0 || isBuy || isLong || isExit) {
                    ...
                    tileBg = '#000000'; // No-data neutral: surface-base (pure black) for non-traded stocks
      ...
      {/* Dynamic Cursor-Following Hover Popover (Borderless #3D3D3D Surface) */}
      {hoveredStock && (
        <div
          style={tooltipStyle}
          ...
          {analysisMode === 'strategy' && signalsMap?.[hoveredStock.symbol] ? (
            ...
            <>
              <div className="flex items-center justify-between gap-2 border-b border-white/[0.08] pb-2">
                ...
                <span
                  className={`font-bold text-xs tabular-nums px-2 py-0.5 rounded-full shrink-0 ${
                    hoveredStock.returnPct >= 0
