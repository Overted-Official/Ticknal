...
export default function ChartViews({ children, positionsView }: ChartViewsProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 relative">
      {/* Mobile Tab Rail */}
      <div className="lg:hidden flex items-center p-2 bg-tv-surface border-b border-tv-border space-x-2 shrink-0">
        <button
          onClick={() => setView('chart')}
          className={`flex-1 py-1.5 text-xs font-weight-medium rounded-tv-full transition-colors ${
            currentView === 'chart' 
              ? 'bg-tv-accent text-white shadow-sm' 
              : 'bg-tv-base text-tv-muted hover:text-tv-text'
          }`}
        >
          Chart
        </button>
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col min-h-0">
        ...
        <div className={`absolute inset-0 flex flex-col bg-tv-base ${currentView === 'positions' ? 'opacity-100 z-10 pointer-events-auto' : 'opacity-0 -z-10 pointer-events-none'}`}>
          {positionsView}
        </div>
      </div>
    </div>
