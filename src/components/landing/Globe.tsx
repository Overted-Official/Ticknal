            ...
            try {
                ...
                if (showOutline && outlineColor && outlineRgba.a > 0) {
                    ...
                    landFeatures.features.forEach((feature: GeoFeature) => {
                        ...
                        processedCount++;
                        const pathString = pathGenerator(feature as Parameters<typeof pathGenerator>[0]);
                ...
                const projection = geoEquirectangular().fitSize(
                    ...
                    { type: "Sphere" } as Parameters<ReturnType<typeof geoEquirectangular>["fitSize"]>[1]
                );
                ...
                ctx.beginPath();
                landFeatures.features.forEach((feature: GeoFeature) => {
                    pathGenerator(feature as Parameters<typeof pathGenerator>[0]);
                });
                ctx.fill();
...
// Minimal GeoJSON feature type to avoid `any` in the callbacks
