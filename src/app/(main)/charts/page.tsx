...
async function ChartsPageContent({
  ...
  const tickerPositions: TickerOrder[] = tickerPositionsData.map((o: any) => {
    ...
    const effectivePrice = isOpen ? currentPriceForSymbol : (o.exitPrice ? Number(o.exitPrice) : entryPrice);
    const profitLoss = (effectivePrice - entryPrice) * quantity;
    const profitLossPct = entryPrice > 0 ? ((effectivePrice - entryPrice) / entryPrice) * 100 : 0;
