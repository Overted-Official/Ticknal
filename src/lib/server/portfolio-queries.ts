...
import {
  ...
  type PositionItem,
  type IndustryGroupStake,
} from '@/types/bank';
import {
  type SectorDataItem,
  type MonthlyDataItem,
  type OrderStats,
  type HomeInvestmentOrder,
  type DashboardOrder,
} from '@/components/platform/home/investments/homeInvestmentsTypes';

export const emptyOrderStats: OrderStats = {
  openOrders: [] as HomeInvestmentOrder[],
  openMarketValue: 0,
  openCostBasis: 0,
  unrealized: 0,
  realized: 0,
...
export async function getOrderStats(userId: string): Promise<OrderStats> {
  try {
    ...
    for (const order of openRows) {
      ...
      if (openOrdersMap.has(symbol)) {
        ...
        openOrdersMap.set(symbol, {
          ...
          entryDate: typeof order.entryDate === 'string' ? order.entryDate : new Date(order.entryDate).toISOString(),
          entryPrice,
          quantity,
          currentPrice,
          profitLoss,
    ...
    const getIsoDate = (d: any) =>
      typeof d === 'string' ? d : new Date(d as unknown as string).toISOString().split('T')[0];
    ...
    for (const order of closedRows) {
      ...
      bucket.realizedPL += pnl;
      bucket.closedCount += 1;
      if (pnl > 0) bucket.closedWins += 1;
    }
...
async function getPortfolioPerformanceMetrics(
  ...
  try {
    ...
    for (const row of rowList as Array<Record<string, unknown>>) {
    ...
    for (const position of closedRows) {
      ...
      if (tradeBars.length > 0 && entryPrice > 0) {
        const adversePrice =
          side === 'SHORT' ? Math.max(...tradeBars.map((bar) => bar.high)) : Math.min(...tradeBars.map((bar) => bar.low));
    ...
    return {
      ...
      avgAdverseExcursion:
        adverseExcursions.length > 0 ? adverseExcursions.reduce((sum, value) => sum + value, 0) / adverseExcursions.length : null,
...
export async function getActiveAlertCount(userId: string): Promise<number> {
  try {
    const rows = await db
      .select({ id: tickerAlerts.id })
      .from(tickerAlerts)
      .where(and(eq(tickerAlerts.enabled, true), eq(tickerAlerts.userId, userId)));
    return rows.length;
  } catch (err) {
...
export async function getUserBankTransactions(userId: string): Promise<BankTransaction[]> {
  try {
    const rows = await db
      .select({
